...
namespace ParticlePlot {
	class Program {
		static void Main(string[] args) {
			...
			Output(args[1], paths, page);
		}
		
		static List<List<Vector2>> ReorderPaths(List<List<Vector2>> paths) {
		...
		static List<Vector2> SimplifyPath(List<Vector2> path) {
		...
		static List<List<Vector2>> SimplifyPaths(List<List<Vector2>> paths) {
			return paths.Select(SimplifyPath).ToList();
		}
		
		static void Output(string fn, List<List<Vector2>> paths, Page page) {
